...
export const validasiBuatKegiatan = [
	...
	body("tanggal_selesai").optional().isISO8601().toDate().withMessage("Format tanggal selesai tidak valid (gunakan format ISO 8601)."),
...
export const validasiEditKegiatan = [
	...
	body("kegiatan_id").isInt({ min: 1 }).withMessage("ID Kegiatan harus berupa angka integer positif.").toInt(),
	...
	body("tanggal_mulai").optional().isISO8601().toDate().withMessage("Format tanggal mulai tidak valid (gunakan format ISO 8601)."),
	...
	body("status").optional().isIn(["Akan Datang", "Berlangsung", "Selesai", "Dibatalkan"]).withMessage("Status kegiatan tidak valid."),

	// Lokasi: Opsional, jika ada, minimal 3 karakter
	body("lokasi").optional().isString().withMessage("Lokasi harus berupa teks.").isLength({ min: 3 }).withMessage("Lokasi minimal 3 karakter."),

	// Kategori ID: Opsional, jika ada, harus integer positif
	body("kategori_id").optional().isInt({ min: 1 }).withMessage("Kategori ID harus berupa angka integer positif.").toInt(),
];

// Validasi untuk menghapus kegiatan (DELETE /kegiatan/:id)
export const validasiHapusKegiatan = [param("id").isInt({ min: 1 }).withMessage("ID Kegiatan yang dihapus harus berupa angka integer positif.").toInt()];
