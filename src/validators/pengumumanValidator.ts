...
export const validasiBuatPengumuman = [
	...
	body("tanggal_penting")
		.optional()
		.isISO8601()
		.toDate()
		.withMessage("Format tanggal penting tidak valid (gunakan format ISO 8601 YYYY-MM-DD)."),

	// Masa Berlaku Sampai: Opsional (kapan kadaluarsa), jika diisi harus format DateTime yang valid
	body("masa_berlaku_sampai")
		.optional()
		.isISO8601()
		.toDate()
		.withMessage("Format masa berlaku tidak valid (gunakan format ISO 8601)."),

	// is_sticky: Opsional, jika ada, harus boolean
	body("is_sticky")
		.optional()
		.isBoolean()
		.withMessage("Status 'Is Sticky' harus berupa boolean (true/false).")
		.toBoolean(), // Konversi string "true" atau "false" ke tipe boolean

	// audiens_jenjang_id: Opsional, jika ada, harus integer positif
	body("audiens_jenjang_id")
		.optional()
		.isInt({ min: 1 })
		.withMessage("Audiens Jenjang ID harus berupa angka integer positif.")
		.toInt(),
];
...
export const validasiEditPengumuman = [
	...
	body("pengumuman_id")
		.isInt({ min: 1 })
		.withMessage("ID Pengumuman harus berupa angka integer positif.")
		.toInt(),
	...
	body("pesan_singkat")
		.optional()
		.isString()
		.withMessage("Pesan singkat harus berupa teks.")
		.isLength({ min: 10 })
		.withMessage("Pesan minimal 10 karakter."),
	...
	body("is_sticky")
		.optional()
		.isBoolean()
		.withMessage("Status 'Is Sticky' harus berupa boolean.")
		.toBoolean(),
...
export const validasiHapusPengumuman = [
	...
	param("id")
		.isInt({ min: 1 })
		.withMessage("ID Pengumuman yang dihapus harus berupa angka integer positif.")
		.toInt(),
];
